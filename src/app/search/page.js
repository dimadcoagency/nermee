...
import { useSearch } from '@/lib/hooks/useServices';
import { CATEGORIES } from '@/lib/constants';

export default function SearchPage() {
  const [query, setQuery] = useState('');
  const [selectedCategory, setSelectedCategory] = useState('all');
  const { results, loading } = useSearch(query, selectedCategory);
    ...
    <div className="flex flex-col min-h-screen bg-white">
      <header className="bg-white border-b border-nearmee-border px-4 pt-12 pb-3 sticky top-0 z-10">
        ...
        <div className="relative">
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-nearmee-text-sec"
          ...
          />
          {query && (
            <button onClick={() => setQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-nearmee-text-sec">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar mt-3 pb-1">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors ${
              selectedCategory === 'all' ? 'bg-nearmee-coral text-white' : 'bg-nearmee-surface text-nearmee-text-sec'
            }`}
          >
            All
          </button>
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors ${
                selectedCategory === cat.id ? 'bg-nearmee-coral text-white' : 'bg-nearmee-surface text-nearmee-text-sec'
              }`}
            >
      ...
      <main className="flex-1 px-4 py-4 pb-28">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-xs text-nearmee-text-sec mt-1">Try "plumbing", "meals", or "cleaning"</p>
          </div>
        ) : results.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <span className="text-5xl mb-4">😔</span>

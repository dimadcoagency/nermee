...
const MENU_SECTIONS = [
  ...
  },
  {
    title: 'For Merchants',
    items: [
      { icon: '🏪', label: 'Become a Merchant', desc: 'List your services on Nearmee' },
      ...
      <header className="bg-white px-4 pt-12 pb-5 border-b border-nearmee-border">
        ...
        <div className="flex mt-5 border border-nearmee-border rounded-xl overflow-hidden">
          {[
            ...
            <div
              key={label}
              className={`flex-1 py-3 text-center ${i < arr.length - 1 ? 'border-r border-nearmee-border' : ''}`}
            >
          ...
          <div key={section.title}>
            ...
            <div className="bg-white rounded-xl border border-nearmee-border divide-y divide-nearmee-border overflow-hidden">
              {section.items.map(({ icon, label, desc }) => (
                <button
                  key={label}
                  className="flex items-center gap-3 px-4 py-3.5 w-full text-left active:bg-nearmee-surface transition-colors"
                >
                  <span className="text-xl w-7 text-center shrink-0">{icon}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-nearmee-text">{label}</p>
                    {desc && <p className="text-xs text-nearmee-text-sec mt-0.5">{desc}</p>}
                  </div>
